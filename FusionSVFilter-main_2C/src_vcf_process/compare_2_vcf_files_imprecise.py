...
def merge_vcf_files(vcf1, vcf2, vcf_output):
    ...
    with open(vcf_output, 'w') as output_file:
            ...
            if is_same_sv(record1, record2):
                ...
                if record1['CHROM'] == record2['CHROM']:
                    if record1['POS'] < record2['POS']:
                        ...
                        sv2_difference_sv1_list.append(sv2)
                        sv2_j += 1
                        dif2 += 1
                else:
        ...
        while (sv2_j < sv_data2_length):
            print(f"{sv1_i}/{sv_data1_length} {sv2_j}/{sv_data2_length}", end='\r')
            sv2_difference_sv1_list.append(sv_data2[sv2_j])
            sv2_j += 1
            dif2 += 1
        # 将合并后的记录写入文件
        for sv in sv2_difference_sv1_list:
            output_file.write(sv)
        print(f"\n{same},{dif1},{dif2}")
        print(len(sv2_difference_sv1_list))
        print(f"实际运行过程中染色体出现的顺序：{chromosome_list}")
    ##############################

if __name__=='__main__':
    data_dir="../data/vcf_5_tools/CLR_pbmm2/"
    vcf1_name="cutesv-CLR-pbmm2.sorted.vcf"
    vcf2_name="pbsv-CLR-pbmm2.sorted.vcf"
    output_file="output_5_tools_4_difference_ResNet50_2C_96_2_len/PBSV_difference_cmp_to_cuteSV_test.vcf"
    vcf1_path = data_dir+vcf1_name
    vcf2_path = data_dir+vcf2_name
    vcf_output_path = data_dir+output_file
