...
if os.path.exists(data_dir + '/all_n_img' + '.pt'):
    ...
    for chromosome, chr_len in zip(chr_list, chr_length):
        ...
        print("ins and del image name start")
        if os.path.exists(data_dir + 'image_name/' + chromosome + '/insert.txt'):
            print("loading")
            ins_image_name_data_path = data_dir + 'image_name/' + chromosome + '/insert.txt'
            del_image_name_data_path = data_dir + 'image_name/' + chromosome + '/delete.txt'
            with open(ins_image_name_data_path, "r") as txt_file:
                ins_image_name_list = txt_file.read().splitlines()
            with open(del_image_name_data_path, "r") as txt_file:
                del_image_name_list = txt_file.read().splitlines()
        print("ins and del image name end")
        ...
        all_ins_image_name=all_ins_image_name+ins_image_name_list
        all_del_image_name=all_del_image_name+del_image_name_list
    ...
    with open(data_dir+'/all_ins_image_name.txt',"w") as txt_file:
        for ins_name in all_ins_image_name:
            txt_file.write(ins_name+"\n")
    with open(data_dir+'/all_del_image_name.txt',"w") as txt_file:
        for del_name in all_del_image_name:
            txt_file.write(del_name+"\n")
...
all_ins_image_name_set=set(all_ins_image_name)
all_del_image_name_set=set(all_del_image_name)

assert len(all_ins_image_name)==len(all_ins_image_name_set),"SV的ID怎么会重复呢？"
assert len(all_del_image_name)==len(all_del_image_name_set),"SV的ID怎么会重复呢？"

print(f"len(all_ins_image_name):{len(all_ins_image_name)},len(all_ins_img):{len(all_ins_img)}")
print(f"len(all_del_image_name):{len(all_del_image_name)},len(all_del_img):{len(all_del_img)}")
assert len(all_ins_image_name)==len(all_ins_img),"图片的数量和图片名字数量都对应不上，还往下做什么"
assert len(all_del_image_name)==len(all_del_img),"图片的数量和图片名字数量都对应不上，还往下做什么"
...
for index in range(length):
    ...
    if index < len(all_ins_img):
        ...
        torch.save([image, 1], data_dir + "ins/" + str(index) + ".pt")
